...
class DataManager {
    ...
    static func prepareData(data: ForecastWeatherModel?, index: Int, options: [WeatherInfoParameters]? = WeatherInfoParameters.allCases) -> WeatherInfoData? {
        ...
        options?.forEach({ (parameter) in
            switch parameter {
            case .icon:
                paramsToReturn[parameter.rawValue] = getDayIcon(data: currentDayArray )
            case .max:
                paramsToReturn[parameter.rawValue] = getMaxTemp(data: currentDayArray )
            case .min:
                paramsToReturn[parameter.rawValue] = getMinTemp(data: currentDayArray )
            case .day:
                paramsToReturn[parameter.rawValue] = dayString
            case .city:
                paramsToReturn[parameter.rawValue] = data.cityData.name
            case .windSpeed:
                if currentDayArray.isEmpty == false {
                    let param = currentDayArray[0].windData.speed.description
                    paramsToReturn[parameter.rawValue] = param
                }
            case .humidity:
                paramsToReturn[parameter.rawValue] = getHumidity(data: currentDayArray )
            }
        })
    ...
    static func getAllWeatherOfDay(data: ForecastWeatherModel?, _ ofDay: String? = nil) -> [ForecastWeatherListModel?]? {
        
        let currentDay = ofDay != nil ? ofDay : Double(Date().timeIntervalSince1970).dateStringFromUTC()
        
        let currentDayArray = data?.weatherList.filter({$0.date.dateStringFromUTC() == currentDay})
...
extension DataManager {
    ...
    private static func getMinTemp(data: [ForecastWeatherListModel]) -> String {
        var temp = 0
        data.forEach { (model) in
            if Int(model.mainWeatherData.temp) < temp {
                temp = Int(model.mainWeatherData.temp)
            }
        }
